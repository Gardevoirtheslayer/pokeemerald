...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_ROSELIA);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForLotad;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForRoselia(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
...
{
    return CreateMonPicSprite_Affine(SPECIES_ROSELIA, SHINY_ODDS, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
